            ...
            >
              ...
              <div className="text-left">
                <div className="flex items-center gap-1">
                  <h1 className="text-lg font-bold bg-gradient-to-r from-red-400 to-red-300 bg-clip-text text-transparent font-display tracking-tight">
                    ELANG SAKTI
                  </h1>
                </div>
              </div>
              ...
              {navItems.map((item, index) => (
                <div key={item.id} className="relative">
                  <button
                    ...
                    className={`relative px-4 py-2.5 text-sm font-medium transition-all duration-300 group ${
          ...
          >
            <div className="w-6 h-6 flex flex-col justify-center items-center gap-1.5">
              <span className={`block h-0.5 w-5 bg-red-300 transition-all duration-300 ${
                isMenuOpen ? 'rotate-45 translate-y-2' : 'group-hover:bg-red-200'
              }`}></span>
              <span className={`block h-0.5 w-5 bg-red-300 transition-all duration-300 ${
                isMenuOpen ? 'opacity-0' : 'group-hover:bg-red-200'
              }`}></span>
              <span className={`block h-0.5 w-5 bg-red-300 transition-all duration-300 ${
                isMenuOpen ? '-rotate-45 -translate-y-2' : 'group-hover:bg-red-200'
              }`}></span>
            ...
            <div className="relative">
              {navItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className={`w-full px-4 py-4 text-left transition-all duration-300 group relative overflow-hidden ${
                    activeSection === item.id
